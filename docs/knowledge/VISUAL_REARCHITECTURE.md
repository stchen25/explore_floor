...
- *Data:* per-job content is **placeholder** pending ARM (the `Job_Program_Data_Request.md` per-job blocks); titles are real (from `commonJobTitles`). `data-integrity` covers the job shapes (counts 3/5/5, non-empty fields, unique ids) + the `explore` copy block.
