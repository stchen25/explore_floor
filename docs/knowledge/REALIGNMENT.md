...
6. **Decide the dormant-classic question and log it.** Recommended: archive the three-archetype and robot and conveyor code to a tagged branch, delete it from `main`, and update `data-integrity.test.ts` to stop validating it. Then drop the "exactly three role families" hard rule and split `types.ts`.
...
- **The Fivestar role overhaul.** Their June 22 update standardized the live site to three roles plus three AI-prefixed variants, and quietly made their own quiz scenario-based. That breaks our four-category mapping. The team already decided not to rework mappings this round (little runway to redo them before the handoff). The realignment should not bake the four-category model so hard that a later "AI versus non-AI screener, then collapse Operate and Repair" path becomes expensive. Keep `roleDetails.ts` and the category mapping easy to re-cut.
...
- **Frame Operator as a starting rung, not a verdict.** Both participants who landed on Operator felt deflated. Pair the entry-level result with a visible path up and outbound links.
