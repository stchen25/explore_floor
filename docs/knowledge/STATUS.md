...
- **Next up:** the **ecosystem run** (`ECOSYSTEM_RUN.md`, ratified 2026-07-02, D-035) — single-session passes, **1–6 done** (per-pass detail lives in the ledger + session notes). Highlights: `@rc/ui` live (tag `v1`), `robotics_career` stood up + tokenized (its D-006), and **Pass 6 (this repo's pre-sync pass) complete on `main`** (D-036/D-037): control heights on the dashboard's token ladder, the full Montserrat weight-honesty sweep, and `DESIGN_SYSTEM.md` reconciled code-outward (RC-CC retired as token source). **Next: Pass 7** (Figma: dark variables into the DS library, the Interest Quiz file, capture) — feeding the **late-July client handoff** (`HANDOFF_GUIDE.md`). Awaiting Caelan: the robotics_career Pass-5 push, the career_dashboard archive tags/branch deletion, and the flagged stale "Figma wins" line in two harness files (Pass-6 note §flags).
