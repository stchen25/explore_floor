...
- **Next up:** the **ecosystem run** (`ECOSYSTEM_RUN.md`, ratified 2026-07-02, D-035) — executed as single-session passes. Passes 1–4 are done: assets rescued; **`@rc/ui` is live** (`caelar/rc-design-system`, tag `v1`); **`robotics_career` is stood up** (`caelar/robotics_career` private) with ARM's six-role Landing; and **its harness is ported** (main @ `5899ca0` local) with a baseline `/design-review` recorded — 75 of 82 hex literals map 1:1 to tokens, so **Pass 5 (tokenization) is next and largely mechanical**; two design-intent calls await Caelan's refinement (gold-as-CTA vs teal-acts; red feedback vs no-red status). Awaiting Caelan's pushes: the robotics_career harness commit + the career_dashboard archive tags/branch deletion. Then Passes 6–7 — all feeding the **late-July client handoff** (`HANDOFF_GUIDE.md`). No in-flight build thread in this repo — branch `narrative-v3-realign` is committed and green.
