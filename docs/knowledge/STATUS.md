...
- **Next up:** the **ecosystem run** (`ECOSYSTEM_RUN.md`, ratified 2026-07-02, D-035) — executed as single-session passes. Passes 1–2 are done: assets rescued, and **`@rc/ui` is live** at `Prototypes/rc-design-system` (main @ `4dae373`, tag `v1`, both gates PASS, pushed to `caelar/rc-design-system` private). **Pass 3 is next: excavate Kayla's branches into `robotics_career`** — blocked on Kayla coordination (branch intent, the "RC.org Prototype" file key, Landing content calls). Then the harness port and the Figma work — all feeding the **late-July client handoff** (`HANDOFF_GUIDE.md`). No in-flight build thread in this repo — branch `narrative-v3-realign` is committed and green.
