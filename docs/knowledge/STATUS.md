...
- **Next up:** **Realignment sweep underway** (`docs/knowledge/REALIGNMENT.md` §9, the plan of record). Branch `narrative-v3-realign`. Done so far: the foundation slice (V3 committed, `/revise-doc`+`doc-steward` ported, tokens kit-aligned, D-024) and **Phase 2, the spec re-centering (D-025)** — every spec doc (`PRD`/`DATA_MODEL`/`DESIGN_SYSTEM`/`ARCHITECTURE`/`ROADMAP`/`CLAUDE.md`/`README`) now leads with the narrative quiz; the conveyor, robot, three-archetype, and Goose-game material is demoted to clearly-marked **"documented cut."** `DATA_MODEL.md` §17 is the primary model (not renumbered). Verified by a 4-skeptic adversarial workflow + `doc-steward`. **Phase 3 done (D-026):** the harness gates now grade the live product — `verifier` + `data-author` headline the §17 flow invariants (classic demoted to a footnote); `goose-game-aesthetic` + `motion-quality` rubrics retired (motion folded into `design-system-compliance`, which is rewritten to the four-category/kit world); a new `results-screen` rubric grades the node-map/dashboard; `scene-motion` rescoped to the live motion. Verified by a 2-skeptic workflow, zero findings. **PAUSED HERE for user go-ahead.** The back half was **re-sequenced 2026-06-25** after two new calls: the prototype goes **narrative-flow-exclusive**, and ARM's live site collapsed our four categories to **three roles** (`docs/reference/ARM Updated Role Structure - Source Content.md`). So Phase 4 is now **Phase 4 (strip to narrative): archive *both* the dormant Classic code *and* the Exam flow** (Exam can't stay dormant — the role pivot changes the shared `CategoryId`), build lands narrative-only on four categories, E2E 7 → ~3, **Select stays**. Then **Phase 5 (pivot): collapse the four categories to ARM's three roles** (Operate+Repair → entry Technician, Program → Specialist, Plan → Integrator), rename the `CategoryId` literals to `technician/specialist/integrator`, and **rewrite** (not drop) the `CLAUDE.md` hard rule to the live three roles. **The full executable plan + the verified Exam-archive blast radius + the corrected order is `sessions/2026-06-25-strip-to-narrative-and-three-roles.md` — read it first; it extends `sessions/2026-06-25-realignment-phases-2-3.md` (still the classic KEEP/DELETE source).** Load-bearing carry-overs: `gsap.ts` LIVE; `/src/scene` survives via `LandingSceneHint` (`RobotPlaceholder` now goes with Exam+Classic). Open decision to settle before deleting: keep `categoryBreakdown.ts` (the match-explanation engine step 8 wants) vs delete with Exam. The old conveyor "feel pass" stays **superseded**; the real next product work is the high-fidelity narrative results screen on three roles.
