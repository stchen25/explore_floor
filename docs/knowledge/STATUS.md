...
- **Next up:** the **ecosystem run** (`ECOSYSTEM_RUN.md`, ratified 2026-07-02, D-035) — single-session passes, **1–7 core done** (per-pass detail lives in the ledger + session notes). Highlights: `@rc/ui` live (tag `v1`), `robotics_career` stood up + tokenized (its D-006), Pass 6 pre-sync complete (D-036/D-037), and **Pass 7 core complete (D-039)**: the dark extension **published** in the DS library (24 variables + 2 effect styles with lineage descriptions, Foundations pages rendering them), the **Interest Quiz file** live in the ARM team with **reference stills of all 8 final screens**, `docs/figma/FIGMA_MAP.md` seeded as the binding manifest both Figma commands read first, and the doc canon **flipped to the three-artifact precedence** (library wins values / `DESIGN_SYSTEM.md` wins usage / code wins behavior; new §16 keeps + §17 `@rc/ui` v1.1 proposals; the two harness lines now defer to §2). **Remainder:** the editable variable-bound screen rebuilds (beside their `Ref/*` twins), the UX repo's capture-target file, the Claude Design `.design-sync` bundle, then stretch — feeding the **late-July client handoff** (`HANDOFF_GUIDE.md`). Awaiting Caelan: the robotics_career Pass-5 push and the career_dashboard archive tags/branch deletion; the dashboard's own FIGMA_MAP §4 totals line (81 vars) is now stale post-append (105) — a dashboard-session fix.
