...
> **Phase C/D scope (D-029).** The results are the dark **role-cards** screen (the first of a 5-screen system: cards → compare → map → constellation → job). The cards screen (Phase C) carries the hero (match label, role name, match %, ranked **signal bars**), an inline **"why you matched"** breakdown (01 what you chose / 02 how they connected / 03 what it means / what you passed on), and two tabs (The role / Skills, path & next steps). **Compare (Phase D)** sets the current role side-by-side with a switchable target (dropdown), each column reusing the hero + "why" + description + salary/education + duties, plus a soft **"Our take"** recommendation line (leads with fit, foregrounds the lower-barrier role when close; no swap). Map / constellation / job land in Phases E–F. Judge cards + compare against the criteria below; the node-map is retired as the headline.
