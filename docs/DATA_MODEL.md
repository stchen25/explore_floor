...
/src/data
  ...
  jobs.ts               LIVE (§17, D-029 Phase F) — per-role featured jobs, 3/5/5 (placeholder copy)
...
Tests for the live lib functions live in `/src/lib/__tests__/` (74 tests across `categoryScoring`, `screenerFit`, `categoryBreakdown`, `compareRecommendation`, `bubbleLayout`, `constellationLayout`, `nodeLayout`, and `data-integrity` — the last now covers the §17 explore copy block + the per-role job invariants 3/5/5). They run as part of the standard `pnpm test` and are independent of Playwright.
