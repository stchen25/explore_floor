...
- **Study flows (`flows/`, §17):** narrative = 7 scenes × 4 choices (one per category), branch targets resolve forward; exam = 30 statements counted 8/7/7/8, interleaved, 3 buckets; each `CategoryFlow.expectedCategoryMax` must equal the computed full-path max (`data-integrity` enforces). Four categories operate/repair/program/plan — parallel to the three archetypes, never mixed. Background-question choices may map to **zero** categories (intentional, D-017). `data-integrity.test.ts` covers all of this per flow.
